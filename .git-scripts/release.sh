...
MINOR_UPDATE=$((VERSIONS[0])).$((VERSIONS[1]+1)).0
MAJOR_UPDATE=$((VERSIONS[0]+1)).0.0

PS3='Please select next version:'
options=($MAJOR_UPDATE $MINOR_UPDATE $PATCH_UPDATE $YOURSELF $QUIT)
if [ -z "$NEXT_VERSION" ]; then
  ...
  do
    if [ $res = $QUIT ]; then
      ...
      git stash pop --quiet
      exit 0
    elif [ $MAJOR_UPDATE = $res ] || [ $MINOR_UPDATE = $res ] || [ $PATCH_UPDATE = $res ]; then
      NEXT_VERSION=$res
      break
...
git pull --quiet
git checkout -b release/$NEXT_VERSION --quiet
